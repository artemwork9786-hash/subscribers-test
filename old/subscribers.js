...
function highlightMatch(text, query) {
  ...
  return (
    ...
    escapeHtml(text.slice(0, q.length)) +
    '</span>' +
    escapeHtml(text.slice(q.length))
  );
}
...
function resizeSubsCount(count) {
  ...
  const width = Math.max(40, len * 7 + 12);
...
function renderTable() {
  ...
  // Get perPage value
  const perPage = window.perPage || 1000;

  // Filter by search and dates
  filteredUsers = allUsers.filter(function (u) {
    if (deletedIds.has(u.id)) return false;

    const matchesId = !queryId || String(u.id).toLowerCase().indexOf(queryId.toLowerCase()) === 0;
    const matchesName = !queryName || u.name.toLowerCase().indexOf(queryName.toLowerCase()) === 0;
    ...
    // Filter by header toggles (removed - static labels now)

    return matchesId && matchesName && matchesDateFrom && matchesDateTo;
  });
  ...
  // Limit by perPage
  const limitedUsers = filteredUsers.slice(0, perPage);

  let html = '';
  limitedUsers.forEach(function (u, index) {
    const rowNum = index + 1;
    const dogLabel = u.dog ? 'Да' : 'Нет';
    html += '<tr class="table__row" data-id="' + u.id + '">';
    html +=
...
(function initDogFilterToggle() {
  ...
  daFilter.addEventListener('click', function (e) {
    e.stopPropagation();
    this.classList.toggle('dog-filter-da--active');
    // Здесь можно добавить логику фильтрации
  });

  netFilter.addEventListener('click', function (e) {
    e.stopPropagation();
    this.classList.toggle('dog-filter-net--active');
    // Здесь можно добавить логику фильтрации
  });
})();
...
(function initSelect() {
  ...
  clear.addEventListener('click', function (e) {
    ...
    options[0].classList.add('custom-select__option--selected');
  });
...
// SEARCH INPUTS
// ========================================
document.querySelectorAll('.table__search-input').forEach(function (input) {
  const wrapper = input.parentElement;
  const clearBtn = wrapper.querySelector('.table__search-clear');
  const searchIcon = wrapper.querySelector('.table__search-icon');
...
document.querySelectorAll('.checkbox-item').forEach(function (item) {
  item.addEventListener('click', function (e) {
    e.preventDefault();
    const input = this.querySelector('.checkbox-item__input');
    const label = this.querySelector('.checkbox-item__label');
...
(function initPagination() {
  ...
  items.forEach(function (item) {
    item.addEventListener('mouseenter', function () {
      if (this.classList.contains('pagination__item--disabled')) return;
      const text = this.textContent.trim();
      if (!tooltipEl) {
        tooltipEl = document.createElement('div');
        tooltipEl.className = 'pagination-tooltip';
        document.body.appendChild(tooltipEl);
      }
      tooltipEl.textContent = 'Перейти на страницу ' + text;
