...
function startsWithAnyWord(text, query) {
  ...
  // Разбиваем на слова и проверяем каждое - ищем query как начало слова
  const words = lower.split(/\s+/);
  for (let i = 0; i < words.length; i++) {
    const word = words[i];
    if (word.length > 0 && word.indexOf(q) === 0) return true;
  }
  return false;
}
...
(function initSearchHandlers() {
  ...
  searchInputs.forEach(function (input) {
    const wrapper = input.closest('.table__search-input-wrapper');
    const icon = wrapper ? wrapper.querySelector('.table__search-icon') : null;
    const clearBtn = wrapper ? wrapper.querySelector('.table__search-clear') : null;
    ...
    function updateSearchUI() {
      ...
      // Скрываем лупу только если есть текст
      if (icon) {
        if (hasText) {
    ...
    input.addEventListener('blur', function () {
      // При потере фокуса показываем лупу только если поле пустое
      if (icon) {
